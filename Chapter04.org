...
Rate coding of \AM signals can take the form of: 
 ...
 rate-responder (where neurons fire in each cycle and as the cycle gets shorter the rate increases up to a certain limit - this is common in low CF units in the AN and CN, and onset units in the PVCN.) 

 specialised feature-based rate coding, especially used to encode \BMF preference in \IC neurons.
...
\end{equation}
#+END_LaTeX
\noindent where $R=N\times\mathrm{S}$.    \SI and Rayleigh tests were also calculated using
