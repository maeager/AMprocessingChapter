...
#+BEGIN_LaTeX
  ...
  \mathsf{SI} = \frac{1}{N} \sqrt{\left(\sum_{i=1}^{i=N} \cos \theta_i \right)^2 + \left(\sum_{i=1}^{i=N} \sin \theta_i \right)^2 }
  \end{equation}
#+END_LaTeX
...
#+BEGIN_LaTeX
  ...
  \caption[Simplified temporal responses of ANFs and T stellate cells]{Simplified temporal responses of ANFs and T stellate cells with respect
...
Hz. The carrier frequency was selected from each of the characteristic frequencies
...
\HSR\space \ANFs have a non-linear f_0 response, with \SI peaking around 0.6 at
...
In \LSR\space \ANFs, Figure \ref{fig:fzero}B, the f_0 response map maintains moderate
