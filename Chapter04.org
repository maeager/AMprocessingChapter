...
band-pass \tMTF responses \citep{RhodeGreenberg:1994}.  Low-pass filters
...
Sustained choppers were characterised as changing from low-pass \tMTF to band-pass \tMTF with increasing sound levels.
...
recurrent network connections between other \TS cells \citep{BahmerLangner:2006,ManorRinzelEtAl:1997}.
...
#+CAPTION: [The temporal \fz response in the CNSM model]{The \fz response map of each cell in the CNSM model to \AM tones of different stimulus intensities and units with different \CFs. The colour bar shows the synchronisation index from 0 to 1, with white representing areas with Rayleigh test not statistically significant (p $>0.05$). The \fz stimulus was an \AM tone where the $f_\mathsf{c} = 8.9$ kHz, $f_\mathsf{m} = 150$ Hz, duration 150 ms, 20 ms delay, and 2 ms on-off ramp. \SI calculations were spikes in the last 130 ms of the stimulus. A. HSR ANFs. B. LSR ANFs. C. GLG cells.  D. DS cells. E. TV cells. F. \ChS\space type \TS cells. G. \ChTone\space type \TS cells. H. \ChTtwo\space type \TS cells }
#+LABEL: fig:fzero
   [[../VowelProcessingChapter/gfx/fresponse-vs.pdf]]

#+ATTR_LaTeX: width=\columnwidth
#+CAPTION: [The rate level response in the CNSM model to \AM tones]{The rate level response map of each cell in the CNSM model to \AM tones with a fixed \fm  and of different stimulus intensities. The stimuli are the same as in Figure \ref{fig:fzero}. A. HSR ANFs. B. LSR ANFs. C. GLG cells.  D. DS cells. E. TV cells. F. \ChS\space type \TS cells. G. \ChTone\space type \TS cells. H. \ChTtwo\space type \TS cells }
#+LABEL: fig:fzerorate
   [[../VowelProcessingChapter/gfx/fresponse-rate.pdf]]


The temporal \fz response maps for each cell in the \CNSM model are shown in
Figure \ref{fig:fzero}. The rate level response maps to the same stimuli are
shown in Figure \ref{fig:fzerorate}. Each column represents a single unit's \fz
...
shown in Figure \ref{fig:fzerorate}A and B. \HSR\space \ANFs have a non-linear \fz response, with
...
provide high (around 0.8 \SI) temporal information.  At high intensities, units with
\CFs near the carrier frequency maintain moderate ($0.5 - 0.6$ \SI) temporal
information that is lacking in the \HSR units in this area.  

Figure \ref{fig:fzero}F shows the \GLG cells' \fz response map.  \GLG cells
...
\ref{fig:fzero}B). The \GLG cells' temporal encoding performance was moderate
near the edges of their response area and deteriorated at high SPL to levels
below 0.3 SI.  The diminished temporal information is a result of the smoothing kernel in the \GLG neural
model. Despite this, the GLG cell model is still capable of passing temporal information
to other cells in the microcircuit (mean SI=0.52, p$<0.05$).

Figure \ref{fig:fzero}D shows that the \DS cell \fz response map had a broad
activation and enhanced the temporal information at every level-CF point on the map
...
The \fz response of \TV cells (Figure \ref{fig:fzero}E) shows an enhanced
temporal response but due to its non-linear rate response is only effective and
...
Figure \ref{fig:fzero}F shows the \fz response map of \ChS subtype \TS cell
...
0.800 \SI at 90 dB \SPL. 
The rate response is not stable at sound levels above 60 dB (as shown by the white patches) 
where the firing rate is low.


Figure \ref{fig:fzero}G shows the \fz response map of \ChTone subtype \TS cell
models.
This behaviour exaggerates the characteristic robustness of the \TS cell
\fz response summarised in Figure \ref{fig:AM:ANCNSummary}.
...
Figure \ref{fig:AM:GMTF} shows the \GLG cell rate and temporal \MTF across the
...
Figures \ref{fig:AM:GMTF}A, C and E, showed a wider activation of units compared
to LSR fibres (Figure \ref{fig:AM:LSRMTF}) with no discernible saturation.  The
...
\ref{fig:AM:GMTF}B, units furthest from the carrier frequency had greater
...
inputs.  The rate responses of \DS units (Figure \ref{fig:AM:DSMTF}) were wider
...
much lower.  \yellownote{Need reference to rate-responder quote}
...
The three chopper \TS cell sub types were processed 
...
Figure \ref{fig:AM:CSMTF} shows the final \MTF response of the \ChS \TS units in the
...
Figures \ref{fig:AM:ChopperONCFMTF}A and B show the \rMTF and \tMTF of the on CF 
\ChS model.
...
By redrawing the results already shown in Figure \ref{fig:AM:CSMTF} for better
...
Figure \ref{fig:AM:CToneMTF} shows the \MTF response of the \ChTone subtype \TS
...
(Figure \ref{fig:AM:CToneMTF}D and F) to low-pass at 80 dB SPL (Figure
\ref{fig:AM:CToneMTF}B).  The \BMF of the on-CF band-pass filters at 350 Hz at
...
Figure \ref{fig:AM:CTtwoMTF} shows the \MTF responses of the \ChTtwo model of
...
(Figures \ref{fig:AM:CTtwoMTF}B, D and F) is strongest in the lateral side bands
...
Figure \ref{fig:fzero}E showed high \SI values near boundaries of its response
...
Figures \ref{fig:AM:CSMTF}, \ref{fig:AM:CToneMTF} and \ref{fig:AM:CTtwoMTF} show
