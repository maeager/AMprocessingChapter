...
Figure \ref{fig:AM:def} shows the general properties of an AM tone and the response of an AN model filterbank. 
...
\yellownote{Fix the linking sentence}


The basic AM parameters modulation depth, variation in \fc and phase , sound
pressure level (SPL) and  fm, influence synchronisation and average rate, then describe the response
phase.
