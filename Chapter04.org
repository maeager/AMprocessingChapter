...
\AM is the periodic modulation of the sound amplitude of a carrier frequency
...
This chapter presents the \AM
coding behaviour of the \CNSM model. It focuses specifically on the temporal
behaviour in response to sinusoidal \AM stimuli.  The
primary output of the \CNSM model are the \TS cells, which provide a robust
...
\citep{CarnevaleHines:2006}, as described in Chapter \ref{chp:Methods}. [[latex:progname][NEURON]]'s
...
Temporal information was measured using the \SI (also called the vector strength) relative to
the modulation frequency of the stimuli.  
\SI and Rayleigh
coefficient were also calculated to verify the \SI values using a fast Fourier transform of the
period histogram.
...
after the onset of the stimulus \citep{JorisSchreinerEtAl:2004}, using
Equation \ref{eq:SI}:
...
The statistical test used to verify the non-uniformity of circular data is
the Rayleigh statistic. The Rayleigh statistic is equivalent to a Chi-squared
test in uniform data and is calculated using $2 n R^2$, where /n/ is
the number of spikes and /R/ is the vector strength.  The critical values for
the Rayleigh test are 5.991 for \alpha = 0.05, 9.21 for \alpha = 0.01, and
13.816 for \alpha = 0.001
\citep{Mardia:1972,ShannonZengEtAl:1995,MardiaJupp:1999}.  The p-values were
...
Sinusoidal \AM tones with fixed carrier and modulation frequencies (f_c = 8.9 kHz
and f_m = 150 Hz) were presented to the \CNSM model to measure the f_0 response.
...
60 dB above threshold \citep{JorisYin:1992}.  The \citet{ZilanyBruceEtAl:2009}
AN model was designed to replicate \AM responses and the f_0 response.  Figures
\ref{fig:fzero}A and B  shows the f_0 response map of the HSR and LSR cell types the \CNSM
model. The V-like f_0 response map of the \HSR fibres (Figure
\ref{fig:fzero}A) shows the effects of rate and temporal saturation at high intensities when the carrier frequency is centred on a unit's CF.
For the unit on CF (8.91 kHz), the peak SI was 0.775 at 15 dB SPL then fell below 0.1 and below the Rayleigh coefficient threshold (5.991) for statistical significance ($\alpha < 0.05$).    
These results are similar to the model results presented in \citet{ZilanyBruceEtAl:2009}.  
The peak SI in the map was 0.777 by unit 84 (CF 20.7 kHz) at 90 dB
...
#+BEGIN_LaTeX
  ...
  \caption[The f0 response in the CNSM model]{The $f_0$ response of each cell in
    ...
    coefficient below 5.991 ($\alpha < 0.05$).  $F_0$ stimulus AM tone duration
    150 ms, delay 20 ms, $f_c = 8.9 kHz$ and $f_m = 150 Hz$, 2 ms on-off
    ramp. Each column represents a single unit's $f_0$ response to the AM
    stimulus.  Each row represents a CF vs. SI curve of a single presentation of
...
ranged from 50 to 1200 Hz in 50 Hz steps.  Each figure shows the mean firing rate (rMTF) on the left
and the synchronisation index (tMTF) on the right. 
The SI values were masked in white if the Rayleigh coefficient were not above 13.681 (\alpha < 0.001).  

# The sound level of each stimulus
# was set to 40 dB \SPL for the top row and 60 dB \SPL for the bottom row.

*** ANF Model Results: ANF Responses to changes in modulation frequency 

Figure \ref{fig:AM:HSRMTF} shows the rate and temporal \MTF maps of \HSR \ANFs
to \AM tones with $f_c=8.91$ kHz.  \HSR fibres saturate to pure \CF tones at 40
...
rate response to AM tones of varying f_m at stimulus intensities 80, 60 and 40 dB
SPL, respectively. The spread of excitation is consistent across f_m at each
intensity.  For HSR units with CF's above f_c, rate increases with f_m and peaks
around 600 Hz.  The rate peak occurs in unit 75 (CF 14 kHz) with rate 30\%
greater than the unit closest to f_c (unit 65, CF 8.9 kHz).
...
excitation and provides little to no information at units with CF's near the f_c frequency.
  ...
  \begin{figure}[thb] 
   ...
   % {\hfill{ Rate (sp/s)\hfill Temporal}}\\
    ...
    \caption{Rate and temporal modulation transfer functions (MTF) of HSR auditory
      nerve fibres at 40, 60 and 80 dB SPL to AM tones with carrier frequency 8.9
      kHz. Rate (right column) and temporal (left) MTF maps of modulation
      frequency ($f_m$) against the CF of the HSR units. Average rate MTF maps
      for stimulus intensities A. 80 dB SPL, C. 60 dB SPL, E. 40 dB SPL.  The
      corresponding temporal MTF maps show their SI masked by a Rayleigh
      coefficient threshold. }  \label{fig:AM:HSRMTF}
  \end{figure}
#+END_LaTeX
...
#+BEGIN_LaTeX
  ...
  %  {\hfill{ Rate (sp/s)\hfill Temporal}}\\
    ...
    \caption[LSR modulation transfer functions]{LSR ANF rate and temporal modulation transfer functions (MTF) of HSR auditory
      nerve fibres at 40, 60 and 80 dB SPL to AM tones with carrier frequency 8.9
      kHz. Rate (right column) and temporal (left) MTF maps of modulation
      frequency ($f_m$) against the CF of the HSR units. Average rate MTF maps
      for stimulus intensities A. 80 dB SPL, C. 60 dB SPL, E. 40 dB SPL.  The
      corresponding temporal MTF maps show their SI masked by a Rayleigh
      coefficient threshold. }  \label{fig:AM:LSRMTF}
  \end{figure}
#+END_LaTeX

\LSR fibers' rate responses to AM tones was non-saturating and centred on the
...
in cat ANFs \citep{JorisYin:1992}.  The low-pass tMTF is typical of \ANFs and
can be seen at each stimulus intensity.  Unusual behaviour in Figure
\ref{fig:AM:LSRMTF}B at high f_m for units with CFs below the carrier frequency
may relate to the rate fluctuations in Figure \ref{fig:AM:LSRMTF}A.
At 80 dB SPL (Figure \ref{fig:AM:LSRMTF}B), the mean SI for the on-CF unit
(unit 65, CF 8.9 kHz) was 0.40, whereas at the outer edge of rate responses
units 77 (CF 15.95 kHz) and 78 (CF 16.67 kHz) both had a mean SI of 0.75.



#+name: mean_vsSPIKES_onCF
#+BEGIN_SRC sh :export none :results none
  grep -e '^[[:digit:]]*[[:space:]]*65' ~/Work/cnstellate/TStellate2_CS/ModulationTransferFunction/80/vsSPIKES.5.dat | \
  awk 'BEGIN{count=0;total=0;} {total+=$4; count+=1} END {printf("%0.3g",total/count)}'
#+END_SRC


*** Golgi cell model responses to  changes in modulation frequency
...
AM tone centred at 8.9 kHz. The GLG cell model's r\MTFs were typically low-pass  mimicking the rate behaviour of \LSR, its primary source of excitation.
The Golgi cell units had very low rates for 40 and 60
...
unit had a flat low-pass tMTF before its Rayleigh
coefficient dropped below the threshold. Its mean SI was 0.571, 0.376, 0.295, 0.218
at stimulus intensities 20, 40, 60, and 80 dB SPL. 
  At the highest intensity, 80
dB \SPL in Figure \ref{fig:AM:G}B, units furthest from the carrier frequency had
greater temporal information (max SI 0.715, unit 77 CF 15.27 kHz), however with
firing rates near threshold their effects on other neurons in the \CNSM model are
predominantly rate-based.
...
#+BEGIN_SRC sh :export none :results raw replace
...
\clearpage
*** DS Model Results: D Stellate Cell Responses to Sinusoidal Amplitude Modulation
...
#+BEGIN_LaTeX
  ...
  \resizebox{\columnwidth}{!}{\includegraphics{../VowelProcessingChapter/gfx/ratetemporal-0.pdf}}
  \caption{TS cell rate and temporal MTF responses for stimulus sound
    levels 80 dB SPL to 20 dB SPL (bottom row).}
  \label{fig:AM:TS}
  \end{figure}
#+END_LaTeX
...
The results demonstrate the capacity to simulate \AM coding over the whole \CN
...
have little to no temporal response to AM tones.
The results in Figure \ref{fig:AM:G} show that the rate response to \AM tones is
only dependent on the sound level. 

The temporal \MTF response of the \GLG cell model
is negligible.


\GLG cells influence the general excitability of \DS and \TS units using \GABAa
synapses on their distal dendrites. 
GABAergic distal inhibitory \PSPs have not been shown in /in vitro/ studies \citep{FerragamoGoldingEtAl:1998a}, but application of bicuculline significantly increased the number of \APs in \DS and \TS cells.
This slow shunting inhibition reduces the amount of \ANF excitation reaching the stellate cells' soma and its site of \AP activation.
At high \SPL, \HSR units are rate-saturated and their temporal \MTF response diminishes with increasing sound level.
...
echo-suppression \citep{WickesbergOertel:1990} in \VCN units, but can also be involved in
tuning the temporal behaviour in \TS cells at low sound levels.

In general, knowledge of the role of \TV cells in temporal processing is incomplete.
\TV cells' temporal response properties are complex and non-linear 
...
#+BEGIN_LaTeX
  ...
  \caption{The average firing rate response of all units in the CNSM model to AM tones of increasing intensity  ($f_c = 8.9 kHz$ and $f_m = 250 Hz$). }  
  \label{fig:Fzerorate}
  \end{figure}
#+END_LaTeX
