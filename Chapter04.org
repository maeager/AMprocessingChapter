...
the edges of the response area .
...
measures used to assess the neural output to AM stimuli and the current
knowledge of responses in the \AN and neurons of the \CNSM model to AM.  The
basic AM parameters modulation depth, and variation in \fc are not included in
this study.  The main focus of this chapter is investigating the responses of
the \CNSM model to variation in sound pressure level (SPL) and \fm using
measures of envelope synchronisation and average rate.
...
Caution must be made to a avoid onset adaptation effects in auditory neurons.
Recordings are performed using long stimuli so that the units are in a
steady-state.  between 20 ms and the end of the stimulus.
...
stimulus feature, as shown in other in sensory pathways.  Rate coding in the
...
The degree of phase locking is measured by the \SI (also called vector strength)
...
were optimised in Section \ref{sec:Chapter3} to fit experimentally observe
behaviour of each of the cell types in the model.  %optimisation of the
parameters % in the CN stellate model is the subject of the doctoral research of
% Michael Eager.
