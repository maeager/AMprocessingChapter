...
The coding of \AM in auditory neurons is measured using a \MTF, which is calculated using
its discharge firing rate (\rMTF) or temporal information (\tMTF). The degree of phase
...
*** GLG Model Results: Golgi Cell Responses to Sinusoidal Amplitude Modulation
...
The broad range of \CF inputs to \DS units allows for a greater likelihood of
