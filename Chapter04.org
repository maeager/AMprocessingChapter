...
study.  As described in Chapter \ref{sec:MethodsChapter}, the Nordlie approach to reproducible neural network simulations
\citep{NordlieGewaltigEtAl:2009} was followed.  The parameters of the \CNSM model
...
\PVCN but the corner- and cut-off frequencies of the \DS cell's \rMTF is much
lower.
...
information at the carrier frequency units.  The rate \MTF responses on the on
CF TV cell in Figure \ref{fig:Ch4:TVonCF}A showed the severe drop in spike
output after 20 dB SPL.  This non-linearity in TV cell responses to AM tones
also make experimental AM tone responses inconclusive
...
\yellownote{TV MTF results}
The temporal \MTF responses of \TV cells were 
predominantly band-pass, with higher
\SI values than \ANFs.  

For lower \SPL, the responses were consistent across
active units with a falling cut-off frequency with falling \CF.  For high \SPL,
...
\DS units above the central channel had the strongest synchronisation and
cut-off frequencies near the upper limit of the \AN model.  

The central channel had corner-off frequencies around 500 Hz (Figure \ref{fig:Ch4:TVonCF}B), indicating a limitation
in the intrinsic cell properties that integrate ANF inputs.
...
The on-\CF\space unit's \rMTFs (Figure \ref{fig:Ch4:ChopperONCFMTF}A) are non-linear and decrease their activity with increasing sound level.
