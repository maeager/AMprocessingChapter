...
\ref{fig:fzero}A and B shows the f_0 response map of the HSR and LSR cell types
the \CNSM model. The V-like f_0 response map of the \HSR fibres (Figure
\ref{fig:fzero}A) shows the effects of rate and temporal saturation at high
intensities when the carrier frequency is centred on a unit's CF.  For the unit
on CF (8.91 kHz), the peak SI was 0.775 at 15 dB SPL then fell below 0.1 and
below the Rayleigh coefficient threshold (5.991) for statistical significance
($\alpha < 0.05$).  These results are similar to the model results presented in
\citet{ZilanyBruceEtAl:2009}.  The peak SI in the map was 0.777 by unit 84 (CF
20.7 kHz) at 90 dB
...
excitation with a low-pass \tMTF extending beyond 1.2 kHz (the maximum range f_m
is this study).  At 80 dB SPL (Figure \ref{fig:AM:HSRMTF}B) units with CFs 15.26
kHz to 18.2 kHz had mean SI of 0.75 with a peak 0.814.  Moving toward f_c the
...
can be seen at each stimulus intensity, particularly in units above the f_C. The
...
and peak SI values than HSR units (mean 0.74, max 0.937) above f_c and the
...
temporal response channels.  Below f_c, an unusual temporal response at high f_m
...
#+BEGIN_LaTeX
  ...
  \caption{Rate and temporal MTF responses for the ChS TS cell model.}
  \label{fig:AM:TS}
  \end{figure}
#+END_LaTeX
...
#+BEGIN_LaTeX
  ...
  \caption{Rate and temporal MTF responses for the ChT 1 TS cell model.}
  \label{fig:AM:CTone}
  \end{figure}
#+END_LaTeX
...
#+BEGIN_LaTeX
  ...
  \caption{Rate and temporal MTF responses for the ChT 2 TS cell model.}
  \label{fig:AM:CTtwo}
  \end{figure}
#+END_LaTeX
