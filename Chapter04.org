...
steadily rises to 0.800 \SI at 90 dB \SPL.  The rate response (Figure
\ref{fig:Ch4:fzero}F) is not stable at sound levels above 60 dB (as shown by the
white patches in the temporal response) where the firing rate is low.  The
inhibition to the \ChS model is stronger for units with \CFs below \fc.
...
p$<0.05$).  The central channel had corner-off frequencies around 500 Hz (Figure
...
greatly diminished.  The temporally active TV cell units at 60 and 80 dB appear
highly synchronised but this is an artefact of the single spike in each
repetition at onset.
...
optimisation performed in Chapter \ref{sec:Chapter3}.  The output of \TS cells
in this chapter were not ideal
snapshot
