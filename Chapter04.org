...
The inhomogeneous population of \TS cells are classified into
...
microcircuit.
\AM rate responses of \TS units on-\CF were fairly weak but this did not disable
...
higher sound levels for on-\CF\space \AM tones \citep{RhodeGreenberg:1994}.
The band-pass temporal responses of different types of TS cells
...
Figures \ref{fig:Ch4:fzero}, \ref{fig:Ch4:fzerorate}, \ref{fig:Ch4:CSMTF},
 \ref{fig:Ch4:CToneMTF} and \ref{fig:Ch4:CTtwoMTF} show snapshots of the
 possible response characteristics in \TS cell models with a view point across
 all cells in the network.  In the \fz response analysis, the \ChS type \TS cell
...
viewed from the that perspective.  Current \TS cell models have investigated AM
...
\citet{ZilanyCarney:2010} AN model has not been verified with AM tones but the
...
# What can be done better in further work

The major limitation of this chapter arises out of the fixed network parameter
 set generated in Chapter \ref{sec:ModelChapter}. The output of \TS cells in
 this chapter were routinely suppressed at high SPL by strong inhibition from
 \DS cells. Whether the design of the optimisation stimuli used in Chapter
 ...
 still an open question.  Also, the homogeneity of cell-type parameters and
 their responses in the CNSM model limits its realism and further work is
 required to understand variability within cell types of the stellate
 microcircuit.
...
excitatory and inhibitory connections can be both a benefit and a detriment for
