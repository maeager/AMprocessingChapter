...
#+LaTeX_CLASS_OPTIONS: [draft,a4paper,11pt,twopage]
...
Investigating the output responses of neurons in the \CNSM to \AM sounds is
critical to understanding how temporal information is processed and encoded in
the auditory nervous system. \AM is a key feature of speech and other
...
intrinsic cell properties in the cells of the \CNSM model
 The optimal coding of \AM is still provided across the network, more specifically in
...
The statistical test used for verifying the non-uniformity of circular data is
the Rayleigh statistic. The Rayleigh statistic is equivalent to a Chi-squared
test for data in uniform data and is calculated using: $2 n R^2$, where /n/ is
...
the rest of the active units remained stable (low-pass \rMTF). This  behaviour
is called ``rate-responder'' due to the firing-rate being dictated to by shortening of the $f_m$ envelope period. 
This is similar to ideal onset units in the \VCN
...
They influence the general excitability of \DS and \TS units using \GABAa
synapses on their distal dendrites. 
GABAergic distal inhibitory \PSPs were not shown in /in vitro/ studies \citep{FerragamoGoldingEtAl:1998a}, but application of bicuculline significantly increased the number of \APs in \DS and \TS cells.
...
In general, the role of \TV cells in temporal processing is incomplete.
