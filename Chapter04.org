...
## set global variables for in-code blocks #+  PROPERTY: cache yes
...
#+RESULTS[8ce00b5ef52b4ed30831df1b641dac51b7b93f29]:
| space | \ | nil |   |   |   |   |
...
Here we simulate the amplitude modulation (AM) coding behaviour
in a biophysically-realistic neural network model of the CN
stellate microcircuit (previously discussed in Chapter \SS 3). TS cells are the major output of the CN and
...
sounds~\citep{BlackburnSachs:1990,KeilsonRichardsEtAl:1997}.  The
...
\citep{NordlieGewaltigEtAl:2009} is followed in
...
describes the following cells and models:
  ...
  kHz.
  2. The Golgi cell: A {GABA}ergic VCN~marginal shell unit is
  assumed to regulate excitability in the GCD~and core VCN~units
  \citep{FerragamoGoldingEtAl:1998}.  Only one \textit{in vivo}
  study has recorded extracellular data in the marginal shell area
  of the CN~\citep{GhoshalKim:1997}.  The presumed characteristics
  ...
  3. The D~stellate cell: A glycinergic, large multipolar cell
  ...
  4. The Tuberculoventral cell: A glycinergic, type II~{EIRA}~unit
  in the deep layer of the DCN~\citep{SpirouDavisEtAl:1999}.  This
  ...
  5. The T~stellate cell: One of the major output projection cells
...
the AM simulations.
# The Nordlie
# \citep{NordlieGewaltigEtAl:2009} table format splits the tables into Model
...
0.05 ms, and total duration 270 ms. Stimuli were repeated 50 times
and the spike times of all CN neurons recorded.

** Stimulus

Stimulus generation follows Rhode and Greenberg's
\citep{RhodeGreenberg:1994} encoding of amplitude modulation in
...
Zilany model \citep{ZilanyBruceEtAl:2009} is based on many auditory
models from the Carney Lab
\citep{HeinzColburnEtAl:2001,ZhangCarney:2001,Carney:1993}. The
centre frequencies for 100 channels is determined by the logarithmic
Greenwood function \citep{Greenwood:1990} of the basilar membrane in
...
one group \citep{GhoshalKim:1997} without direct labelling of
...
sound intensity \citep{GhoshalKim:1997}.
...
fibres \citep{Jackson:2003,JacksonCarney:2005}.  Spike trains for
...
\VCN neurons in mice
\citep{RothmanManis:2003b}. Their activation and deactivation
functions (/a, b, c, h, m, n, p, r, w/ and /z/) are
described in detail by Rothman and Manis \citep{RothmanManis:2003}
and the NEURON source code is freely available online at ModelDB
\citep{HinesMorseEtAl:2004}.  Table~\ref{tab:Celltypes2} shows the membrane
...
stellate microcircuit.  Single exponential excitatory synapses (\tAMPA = 0.36 ms) model the experimental recordings in VCN neurons
...
synapses \citep{LeaoOleskevichEtAl:2004} (\tGly1=0.4 ms and \tGly2=2.5 ms) and
\GABAa synapses \citep{AwatramaniTurecekEtAl:2005} (\tGABA1=0.7 ms and
\tGABA2=9.0 ms) were modeled from MNTB neurons in mature guinea pigs.  Chlorine
...
the modulation frequency of the stimuli.  The synchronisation index (SI) was
...
#+name: AN_F0
#+BEGIN_SRC gnuplot :exports none :file ./gfx/AN_f0.png :term pngcairo size 700,524 enhanced 
reset
...
plot "< ls -rt /media/c4bb64a6-7c5f-4dc1-9965-b0f4c1117b36/Work-archive/cnstellate-03-Feb-2012/TStellate_CS/ModulationTransferFunction/60/*/vsSPIKES.2.dat| xargs awk '/^50\t/ {print $2, $3}' " u (($0+1)*50):1 w l axes x1y1, \
"< ls -rt /media/c4bb64a6-7c5f-4dc1-9965-b0f4c1117b36/Work-archive/cnstellate-03-Feb-2012/TStellate_CS/ModulationTransferFunction/60/*/vsSPIKES.2.dat| xargs awk '/^50\t/ {print $2, $3}' " u (($0+1)*50):2 w l axes x1y2, \
"< ls -rt /media/c4bb64a6-7c5f-4dc1-9965-b0f4c1117b36/Work-archive/cnstellate-03-Feb-2012/TStellate_CS/ModulationTransferFunction/60/*/rateplace.2.dat | xargs awk '/^50\t/ {print $3}'" u (($0+1)*50):(10*$1) w l axes x1y2
...
#+name: MTF_example2
#+begin_src gnuplot :exports none :file ./gfx/MTF_example.png :term postscript eps size 3.5,2.62 enhanced defaultplex leveldefault colour solid dashlength 1.0 linewidth 2.0 butt noclip  palfuncparam 2000,0.003  "Helvetica" 18
  reset
  load "/media/data/Work/cnstellate/ResponsesNoComp/default.gnu"
...
#+name: MTF_atCF_gnu
#+begin_src gnuplot :exports none :file gfx/MTF_atCF.eps :term post eps size 3.5,2.62 enh color solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
  reset
  load "/media/data/Work/cnstellate/ResponsesNoComp/default.gnu"
...
#+name MTF_example3
#+BEGIN_SRC gnuplot :file gfx/MTF_example.eps :term post eps size 7.00,5.24 enh color solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
#  :term pngcairo enhanced defaultplex  leveldefault color  solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
  reset
...
#+RESULTS[46e4c45c581de8786ec839967c764a832f8bb3fb]:
[[file:gfx/MTF_example.eps]]
...
#+name CS_MTF
#+BEGIN_SRC gnuplot :file gfx/CS_MTF.eps  :term postscript eps size 7.00,5.24 enhanced defaultplex  leveldefault color  solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
  reset
...
#+name CT1_MTF
#+BEGIN_SRC gnuplot :file gfx/CT1_MTF.eps  :term postscript eps size 7.00,5.24 enhanced defaultplex  leveldefault color  solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
  reset
  ## Keep the figure clean
    set border 0          # no borders
    unset key
...
#+name CT2_MTF
#+BEGIN_SRC gnuplot :file gfx/CT2_MTF.eps  :term postscript eps size 7.00,5.24 enhanced defaultplex  leveldefault color  solid dashlength 1.0 linewidth 2.0 butt noclip palfuncparam 2000,0.003 "Helvetica" 12
  reset
  ## Keep the figure clean
    set border 0          # no borders
    unset key
...
#+results: CT2_MTF
file:gfx/CT2_MTF.eps


* Discussion

Golgi cells are low-firing monotonic units that influence the general
excitability of DS and TS units using GABA.  The results in Fig.~\ref{fig:G}
show that the rate and temporal response to AM tones is only dependent on the
sound level.
...
AM rate saturation of TS units on CF (Fig.~\ref{fig:TS}) does not disable their
ability to encode temporal information.  Experimental data has shown TS cells
generally have low-pass MTF at low sound level and band-pass MTF for higher
sound levels for AM tones on CF~\citep{RhodeGreenberg:1994}.  The implications
for the AM coding in TS output on higher-order auditory centres have been
investigated but not fully understood
